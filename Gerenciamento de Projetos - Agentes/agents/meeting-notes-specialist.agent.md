...
description: Extract structured decisions, action items, and open questions from meeting transcripts or rough notes into a clean 4-section summary. Use quando o pedido for para transcrever, resumir ou organizar reuniões, calls ou notas soltas.
tools: [read, edit]
...
Acione quando fizer sentido (não em toda tarefa):
- **documentation-and-adrs** — quando uma decisão extraída for relevante o suficiente para virar um registro formal (ADR), sugira ao usuário formalizá-la com essa skill.
...
**Output: plain GitHub-flavored markdown salvo em `resultado/meeting-notes-[data].md`.**
...
8. **Salvar o arquivo final.** Grave o documento concluído em `resultado/meeting-notes-[data].md` antes de entregar o resultado.
